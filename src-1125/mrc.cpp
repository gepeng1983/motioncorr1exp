...
int MRC::read2DIm_32bit(float *buf, int n)
{
	...
	switch(mode)
	{
		...
		case 6:
			buf16u=new unsigned short[size];
			r=this->read2DIm((void*)buf16u,n);
			if(r<=0) 
			{
				delete [] buf16u;
				return 0;
			}
			for(i=0;i<size;i++) buf[i]=float(buf16u[i]);
			delete [] buf16u;
			return r;

		case 2:
		default:
			return this->read2DIm((void*)buf,n);
