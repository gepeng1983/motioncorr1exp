...
public:	
	...
	int getNy();
	int getNz();
	virtual int getWordLength();   //get word length in byte
	int getMode();
	int getSymdatasize();
	float getMin();
	float getMax();
	...
	virtual int read2DIm(void *buf, int n);
	int read2DIm_32bit(float *buf, int n);
	virtual int readLine(void *buf, int nimage, int nline);
	virtual int readPixel(void *buf, int nimage, int nline, int npixel);
        virtual int readnPixels(void *buf, int nimage, int nline, int npixel,int n);
	virtual int write2DIm(void *buf, int n);
	virtual int writeLine(void *buf, int nimage, int nline);
	virtual int writePixel(void *buf, int nimage, int nline, int npixel);
	virtual void setHeader(const MRCHeader *pheader);
	void setMin(float min);
	void setMax(float max);
	void setMean(float mean);
	virtual void updateHeader();
	
	virtual int createMRC(float *data, int nx, int ny, int nz);
	virtual int createMRC(short *data, int nx, int ny, int nz);
	bool hasFile();
